document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchAndRenderPlugins() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const plugins = (data.plugins && data.plugins.length > 0) ? data.plugins : DEFAULT_PLUGINS;
        renderPluginGrid(plugins);
        return;
      }
    } catch (err) {
      console.warn("Could not fetch plugins from server API, rendering fallback plugins:", err);
    }
    // Fallback if network/API fails
    renderPluginGrid(DEFAULT_PLUGINS);
  }
  ...
  function renderPluginGrid(plugins) {
    ...
    pluginGridList.querySelectorAll(".btn-plugin-toggle").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        // Client-side fallback toggle if server API unreachable
        const target = DEFAULT_PLUGINS.find(item => item.id === pluginId);
        if (target) {
          target.status = target.status === "ACTIVE" ? "INSTALLED" : "ACTIVE";
          renderPluginGrid(DEFAULT_PLUGINS);
        }
      });
    });
  ...
  // Bind initial click listeners to pre-rendered HTML cards in case modal opened before JS fetch
  pluginGridList.querySelectorAll(".btn-plugin-toggle").forEach(btn => {
    btn.addEventListener("click", async () => {
      const pluginId = btn.getAttribute("data-plugin-id");
      try {
